				...
				<c:forEach items="${chanceInfo.list}" var="chance">
					<tr>
						...
						<c:if test="${chance.chcStatus==1}">
							<td>等待开发</td>
							<td><i class="layui-icon layui-icon-loading"
								value="${chance.chcId}"></i></td>
						</c:if>
						<c:if test="${chance.chcStatus==2}">
							<td>正在开发</td>
							<td><i class="layui-icon layui-icon-fonts-code" value="${chance.chcId}"></i></td>
						</c:if>
						<c:if test="${chance.chcStatus==3}">
							<td>开发成功</td>
							<td><i class="layui-icon layui-icon-face-smile"
								value="${chance.chcId}"></i></td>
						</c:if>
					</tr>
				</c:forEach>
					...
					onclick="pageSelect5(1)">首页</a> <c:choose>
						<c:when test="${chanceInfo.hasPreviousPage }">
							<a href="javascript:void(0)"
								onclick="pageSelect5(${chanceInfo.prePage})">上一页</a>
						</c:when>
						<c:otherwise>
						...
						<c:when test="${chanceInfo.hasNextPage }">
							<a href="javascript:void(0)"
								onclick="pageSelect5(${chanceInfo.nextPage})">下一页</a>
						</c:when>
						<c:otherwise>
							<font color="#ABA8AB">下一页</font>
						</c:otherwise>
					</c:choose> <a href="javascript:void(0)"
					onclick="pageSelect5(${chanceInfo.lastPage})">尾页</a>
				</td>
			</tr>
...
function pageSelect5(curPage){
	$('.layui-body').load('plans/findPlansByUserId/'+ curPage);
}
	$(function(){
		var chanceId;
		// 查询 
		$('.search').click(function(){
			...
			console.log(city);
			var url = 'sales/findChanceByAddr/1';
			$('.layui-body').load(url,{chcAddr:city});
		})
	
		$('.layui-icon-loading').click(function(){
			chanceId = $(this).attr('value');
			$('.layui-body').load('plans/toPlan_add/' + chanceId);  
		})
		$('.layui-icon-fonts-code').click(function(){
			chanceId = $(this).attr("value");
			$('.layui-body').load('plans/toPlan_edit/'+ chanceId);  
		})
		$('.layui-icon-face-smile').click(function(){
			chanceId = $(this).attr("value");
			$('.layui-body').load('plans/toPlan_detail/	' + chanceId);  
		})
	})
</script>